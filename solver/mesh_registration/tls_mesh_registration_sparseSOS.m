function solution = tls_mesh_registration_sparseSOS(problem)
    % use sparseSOS to solve TLS mesh registration
    % Heng Yang
    % 03/26/2020
    yalmip('clear')
    % copy data
    ...
    t_relax = toc(t0); 
    %%%%%%%%%%%%%%%%%%%%% enb sparse SOS derivation %%%%%%%%%%%%%%%%%%%%
    % solve the SDP
    options = sdpsettings('solver', 'mosek', 'verbose', 1, 'debug',1);
%     options = sdpsettings('solver', 'cdcs', ...
%         'cdcs.solver', 'primal', 'cdcs.maxIter',10000,'cdcs.relTol',1e-6,...
%         'verbose', 1, 'debug',1);
%     options = sdpsettings('solver', 'sdpnal', ...
%         'sdpnal.maxiter',1000,...
%         'verbose', 1, 'debug',1);
    ...
    allPoints = 1:N;
    detectedOutliers = allPoints(theta_est<0);
    solution.detectedOutliers = detectedOutliers;
